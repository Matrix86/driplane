...
func TestCacheDoFilterWithNamed(t *testing.T) {
	filter1, err := NewCacheFilter(map[string]string{"target": "main", "name": "test"})
	if err != nil {
		t.Errorf("constructor returned '%s'", err)
	}
	filter2, err := NewCacheFilter(map[string]string{"target": "main", "name": "test"})
	if err != nil {
		t.Errorf("constructor returned '%s'", err)
	}
	f1, ok1 := filter1.(*Cache)
...
func TestCacheDoFilterFirstRun(t *testing.T) {
	filter, err := NewCacheFilter(map[string]string{"target": "main"})
	if err != nil {
		t.Errorf("constructor returned '%s'", err)
	}
	if e, ok := filter.(*Cache); ok {
