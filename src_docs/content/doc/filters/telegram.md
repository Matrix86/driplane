...
| **to**     | _STRING_ | ""             | used with action `send_message`, it has to contain the recipient of the message: username, @username, phone number with the country code (supports [templates](../../rules/templates)) |
| **to_chatid** | _STRING_ | empty | the recipient of the message will be a chat |
| **filename** | _STRING_ | "" | specified the path of where to store the downloaded file: `msg_filename` in the extra will contain the name of the file (supports [templates](../../rules/templates)) |
| **text** | _STRING_ | "" | the text of the file (supports [templates](../../rules/templates)) |
