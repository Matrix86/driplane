...
Both the `prompt` and `system_prompt` parameters support [templates](../../rules/templates), allowing you to dynamically compose prompts using the `main` field and any extra fields of the incoming `Message`.
...
| **prompt**        | _STRING_ |          | **(required)** The user prompt sent to the LLM. Supports [templates](../../rules/templates) with `Message` fields             |
| **system_prompt** | _STRING_ | empty    | An optional system prompt. Supports [templates](../../rules/templates) with `Message` fields                                  |
